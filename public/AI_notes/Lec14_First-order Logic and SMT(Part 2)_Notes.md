...
1. $`x_1=store(x_0,0,y_0)`$，所以
   ...
   $`\displaystyle select(x_1,0)=y_0`$

2. $`x_2=store(x_1,1,select(x_1,0))`$，所以

   $`\displaystyle select(x_2,1)=select(x_1,0)`$

3. 因此：

   $`\displaystyle select(x_2,1)=y_0`$

4. 由函数一致性：

   $`\displaystyle sq(select(x_2,1))=sq(y_0)`$
...
1. 等式集合：
   ...
   $`\displaystyle E=\{s_1=t_1,\ldots,s_m=t_m\}`$

2. 不等式集合：

   $`\displaystyle N=\{s_{m+1}\ne t_{m+1},\ldots,s_n\ne t_n\}`$
